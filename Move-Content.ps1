...
function Publish {
    ...
    $postMeta = Get-ChildItem ./ready/posts -Filter *.html -Include $Include -Exclude $Exclude | ForEach-Object {
        $fullPath = $_.FullName
        $baseName = $_.BaseName
...
function Render {
    ...
    Get-ChildItem ./draft -Filter *.md -Include $Include -Exclude $Exclude | ForEach-Object {
        $fullPath = $_.FullName
        $baseName = $_.BaseName
